...
pub const PROVIDER_ID: &str = "jiohotstar";

#[derive(Debug, Clone, Copy, Default)]
pub struct HotstarAdapter {
...
impl HotstarAdapter {
    pub fn login_required(&self, id: u64) -> CdpCommand {
        runtime_command(
            id,
            "Boolean((location.hostname.includes('hotstar.com') || location.hostname.includes('jiocinema.com')) && (document.querySelector('input[type=\"tel\"], input[type=\"password\"]') || document.body.innerText.toLowerCase().includes('log in')))",
        )
    }
    ...
    pub fn detect_media(&self, id: u64) -> CdpCommand {
        runtime_command(
            id,
            "Boolean((location.hostname.includes('hotstar.com') || location.hostname.includes('jiocinema.com')) && document.querySelector('video'))",
        )
    }
    ...
    pub fn media_identity(&self, id: u64) -> CdpCommand {
        runtime_command(
            id,
            "(() => ({ title: document.title || '', content_id: location.pathname.split('/').filter(Boolean).pop() || null, provider: 'jiohotstar' }))()",
        )
    }
...
mod tests {
    ...
    #[test]
    fn hotstar_adapter_has_login_media_and_control_paths() {
        let adapter = HotstarAdapter::default();

        assert!(expression(&adapter.login_required(1)).contains("log in"));
        assert!(expression(&adapter.detect_media(2)).contains("hotstar.com"));
